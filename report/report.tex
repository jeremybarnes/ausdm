...
We describe how we combined 1152 ``black box'' models into a more powerful ensemble predictor.  Several models that achieved significant success on the AUC task were produced: a deep neural network model, a gated classifier and a multiple logistic regression model.  Further improvement was achieved by adding features derived from the model input, and by adding unsupervised features from decomposing the model inputs (using a SVD or denoising auto-encoders).
...
The AUSDM ensembling challenge is based upon the NetFlix Competition \cite{NetFlixPrize} which recently finished.

\subsection{Problem Analysis}

As time and labour are limited, I choose to concentrate mostly on the AUC task, as I feel that this represents better the real-world application of recommendation engines (for example, NetFlix presumably wants to optimize the probability that someone who sees a recommendation rents the film\footnote{Or rents the film and doesn't hate it}, as this reduces the chances that someone lets their subscription lapse.
